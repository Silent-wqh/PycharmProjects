...
def check_events(ai_settings, screen, ship, bullets, stats, play_button, aliens, sb):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keydown_events(event, ship, bullets, ai_settings, screen, stats, aliens)
...
def check_keydown_events(event, ship, bullets, ai_settings, screen, stats, aliens):
    """响应按键"""
...
def create_fleet(ai_settings, screen, aliens, ship):
    ...
    for alien_number in range(get_number_aliens_x(ai_settings, alien.rect.width)):
        create_alien(ai_settings, screen, aliens, alien_number,
                     get_number_rows(ai_settings, ship.rect.width, alien.rect.height))

    # 创建外星人群
    for row_number in range(get_number_rows(ai_settings, ship.rect.width, alien.rect.height)):
        for alien_number in range(get_number_aliens_x(ai_settings, alien.rect.width)):
            create_alien(ai_settings, screen, aliens, alien_number,
                         row_number)
